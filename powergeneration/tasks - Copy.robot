...
Power Generation
    ...
    ${excel_rows}=    Read Worksheet    PowerGeneration
    Log    ${excel_rows}
...
Login to App
    ...
    Safe Click Element    xpath=(//a[contains(text(),'Power Generation')])[1]
    Sleep    10s
    Safe Click Element
...
Fill Pricing Details[First Run]
    ...
    Safe Click Element
    ...    (//input[@name='ctl00$ctl00$ctl00$ctl00$PartContentPlaceHolderMain$ContentPlaceHolderMain$ContentPlaceHolderPolicyMain$ContentPlaceHolderMain$ctl00'])[1]
    Wait Until Element Is Visible    (//a[normalize-space()='Natural Perils Rates'])[1]    20s
    Safe Click Element
    ...    (//input[@name='ctl00$ctl00$ctl00$ctl00$PartContentPlaceHolderMain$ContentPlaceHolderMain$ContentPlaceHolderPolicyMain$ContentPlaceHolderMain$ctl00'])[1]
    ...
    Select From List By Label
    ...    //*[@id="ctl00_ctl00_ctl00_PartContentPlaceHolderMain_ContentPlaceHolderMain_ContentPlaceHolderPolicyMain_ctrlPremiumBreakdownContainer_ddlMajorRatingSegment"]
    ...    Chemical
    Safe Click Element
    ...    (//input[@name='ctl00$ctl00$ctl00$PartContentPlaceHolderMain$ContentPlaceHolderMain$ContentPlaceHolderPolicyMain$ctl00'])[1]
...
Quote
    ...
    ${extra_expenses}=    Get From Dictionary    ${current_row}    BB
    Select From List By Label
    ...    (//select[@id='ctl00_ctl00_ctl00_PartContentPlaceHolderMain_ContentPlaceHolderMain_ContentPlaceHolderPolicyMain_ctrlQuoteFinal_ddlExtraExpenses'])[1]
    ...    ${extra_expenses}
    ${coin_property}=    Get From Dictionary    ${current_row}    BC
    Select From List By Label
    ...    (//select[@id='ctl00_ctl00_ctl00_PartContentPlaceHolderMain_ContentPlaceHolderMain_ContentPlaceHolderPolicyMain_ctrlQuoteFinal_ddlCoInsuranceProperty'])[1]
    ...    ${coin_property}
    ${coin_business_income}=    Get From Dictionary    ${current_row}    BD
    Select From List By Label
    ...    (//select[@id='ctl00_ctl00_ctl00_PartContentPlaceHolderMain_ContentPlaceHolderMain_ContentPlaceHolderPolicyMain_ctrlQuoteFinal_ddlCoInsuranceBusinessIncome'])[1]
    ...    ${coin_business_income}
    ${cancellation_days}=    Get From Dictionary    ${current_row}    BE
    Safe Input Text
    ...    (//input[@id='ctl00_ctl00_ctl00_PartContentPlaceHolderMain_ContentPlaceHolderMain_ContentPlaceHolderPolicyMain_ctrlQuoteFinal_txtCancellationDays'])[1]
    ...    ${cancellation_days}
    ${non_payment_days}=    Get From Dictionary    ${current_row}    BF
    Safe Input Text
    ...    (//input[@id='ctl00_ctl00_ctl00_PartContentPlaceHolderMain_ContentPlaceHolderMain_ContentPlaceHolderPolicyMain_ctrlQuoteFinal_txtNonPaymentDays'])[1]
    ...    ${non_payment_days}
    Sleep    10s
    Safe Click Element
...
Reissue
    ...
    Sleep    10s
    Safe Click Element    //div[@aria-describedby='reissuePolicyDialog']//span[text()='Yes']
    Verify Status
    ...    (//span[@id='ctl00_ctl00_PartContentPlaceHolderMain_ContentPlaceHolderMain_riskHeader_labelStatus'])[1]
...
RA Approval
    ...
    ${NUSER}=    Get From Dictionary    ${current_row}    B
    ${NUSERPASSWORD}=    Get From Dictionary    ${current_row}    C
    Open Available Browser    ${URL_RA1}    maximized=${TRUE}    browser_selection=edge
    Sleep    10s
    Safe Click Element    (//a[@class='btn btn-info'])[1]
...
Safe Click Element
    [Arguments]    ${locator}    ${retries}=10x    ${retry_interval}=2s
...
Safe Input Text
    [Arguments]    ${locator}    ${text}    ${retries}=10x    ${retry_interval}=2s
    Wait Until Keyword Succeeds
    ...    ${retries}
    ...    ${retry_interval}
